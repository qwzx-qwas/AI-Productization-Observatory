...
- 输出结果：面向 Phase0 退出的统一执行清单、真实可审计的 formal `gold_set` 与 adjudication 资产、当前 MVP 参考样本集（`134 gold_set + 75 approved_for_staging + 162 rejected_after_human_review + 28 on_hold`）的分层证据、与当前基线匹配的测试与 gate 证据、同步收口后的 README / artifact / 状态声明，以及一个可继续安全扩写的 Phase0 基线。

补充边界：

- 自 `2026-04-10` 起，Phase0 MVP 以当前已物化的分层参考样本集作为完成基线。
- `gold_set/gold_set_300/` 与 `docs/gold_set_300_real_asset_staging/` 继续保留历史路径名与 carrier 结构，但不再把“补满 300 个槽位”当作 Phase0 MVP 的强制前提。
- 文中出现 `gold_set_300` 时，优先理解为 formal gold set 的现有路径名，而不是一个必须继续追满的样本目标。
...
### 1. 背景
当前仓库已经完成最小工程骨架、语义 contract 对齐、冻结板签字和统一回写，也已具备 deterministic fixture replay、最小 mart build、schema/config 校验与基础测试。当前需要收口的重点，不再是把历史 carrier 强行补满，而是把 formal `gold_set`、`screening_*` 分层样本、文档口径与验证证据统一到同一套 MVP 基线。
...
- 明确当前 Phase0 收口必须完成的对象清单：formal `gold_set`、双标原始结果、adjudication 结果、channel metadata、MVP 参考样本集分层证据、Phase0 量化 gate 计算证据、与当前基线匹配的关键路径测试。
...
- `gold_set/README.md` 当前已进入 `implemented` 状态；后续收口应继续以真实资产状态为准，不得把历史 stub 描述继续当作当前事实。
...
- 边界条件：对当前只有最小 fixture、但 formal gold set 与 screening calibration 仍在分层推进的模块，要明确其“可作为当前基线验证资产”但“不能越权替代 formal gold set”。
- 关键回归点：不把 Task 级完成误写成 Phase 级完成；不把 stub 资产或未计算 gate 误写成已通过。

### 8. 验收标准
- 形成一份只面向 Phase0 正式退出的文件级收口清单。
- 清单中每个缺口都已标明归属文件、依赖关系、是否阻塞 Phase0 退出。
- 明确列出当前可复用的最小基线验证资产，以及 MVP 参考样本集、formal gold set 与 gate 证据之间的职责分工。
...
### 1. 背景
Phase0 的正式退出仍要求 formal gold set 具备双标 + adjudication 资产，但当前 MVP 已不再把“填满历史 300 槽位”视为阻塞。没有真实双标样本、adjudication 结果、原始通道记录与 channel metadata，仍不能计算或证明当前 formal gold set 的质量基线。

### 2. 目标
把 `gold_set/gold_set_300/` 维持在可审计、可回放、可用于 Phase0 MVP gate 计算的真实资产状态，并与 annotation、review、taxonomy、rubric 的 canonical 规则保持一致。
...
- `gold_set/README.md` 中与当前 MVP 参考样本集、formal gold set 与分层边界相关的条件全部被真实资产满足。
...
- 基于当前 formal `gold_set` 与 MVP 分层参考样本集计算并留存 Phase0 所需的 gate 与评估证据。
...
- 不跳过 formal gold set 与分层参考样本集 gate，直接以现有 41 个测试通过替代 Phase0 正式退出。
...
- 当前 formal `gold_set` adjudication complete = `100%`，MVP 参考样本集分层计数与文档一致，且关键路径测试达到 `01_phase_plan_and_exit_criteria.md` 当前 MVP 口径。
...
### 1. 背景
即使 formal `gold_set`、MVP 参考样本集 gate 计算与关键路径测试都已完成，若 README、artifact 状态、gold set 状态、阶段位置和当前验证结论没有统一回写，仓库仍会停留在“讨论过、验证过，但状态未收口”的灰区。Phase0 的正式完成必须以仓库可见的状态更新、证据回链和剩余边界声明结束。
...
- 若 gold set 与 MVP 参考样本集已满足条件，应诚实保持 `implemented` 并同步回写相关结论；若仍有缺口，则明确阻断 Phase0 完成，不得模糊表述。
...
### 5. 需要明确的实现问题
- `gold_set/README.md` 的状态只能随真实资产变化；若 formal gold set 资产不完整或与当前 MVP 参考样本口径冲突，必须阻断 Phase0 完成。
...
- 若成功完成，则：
  - formal `gold_set` 与 adjudication 资产已落成
  - Phase0 MVP 量化 gate 与关键路径测试已通过
