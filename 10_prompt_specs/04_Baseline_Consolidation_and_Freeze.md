...
- 依赖前提：Task 1 的工程资产、Task 2 的语义回写、Task 3 的人工签字结论已经形成；当前仓库中 `fixtures/README.md` 与 `gold_set/README.md` 仍显示 stub，README 也说明“代码实现：尚未开始”。
...
### 1. 背景
Task 4 不仅要收文档，还要让仓库真正变成“可运行基线”。当前代码主链尚处起步阶段，fixtures 与 gold set 目录也尚未真正落内容；同时任务总纲要求补齐 2 到 3 条端到端示例和 API/dashboard contract。
