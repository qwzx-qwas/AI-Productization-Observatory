...
## 任务概述
- 任务目标：把当时“文档与 artifact 已冻结、代码尚未落成”的仓库推进到最小可运行骨架状态，形成真实命令、基础代码入口、最小 fixture、测试壳子、runtime skeleton 与 replay 入口。
- 依赖前提：以 `implementation_execution_tasks_20260327.md` 为任务总纲，以 `document_overview.md` 的 canonical 裁决规则为准；当前仓库已落成该任务的最小代码基线，因此后续执行者应把本文件视为 Task 1 的任务说明与验收边界，而不是继续假设 `src/` 尚未实现或 `fixtures/README.md` 仍为 `stub`。
...
### 1. 背景
仓库已经冻结了 repo 映射、runtime profile、测试矩阵和 prompt/schema/config contract。Task 1 的原始起点是 README 仍说明“代码实现：尚未开始”；当前仓库状态已不再如此，但后续执行者仍应沿用这里定义的工程入口、命令边界和最小骨架要求。
...
### 1. 背景
当前仓库的测试计划、fixture 策略和 acceptance gate 已冻结。Task 1 的原始起点是 `fixtures/README.md`、`gold_set/README.md` 仍是 stub，CI 也尚未落地；当前仓库已落成最小 fixtures 与 CI，因此后续执行者应把这里当作 Task 1 的验收约束，而不是当前状态快照。
