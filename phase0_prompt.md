...
## 7.5 补充执行规则（若任务触及 staging 或 screening calibration assets）

若任务涉及 `gold_set`、`docs/gold_set_300_real_asset_staging/` 或 `docs/screening_calibration_assets/`，执行前必须先做以下核对：

- 当前参考样本基数（MVP 临时口径）固定为：`134 gold_set + 75 approved_for_staging + 162 rejected_after_human_review + 28 on_hold`。
- 为简化当前实施步骤，现阶段暂以这批样本作为参考样本集合，用于先跑通一版 MVP。
- 上述口径是分层参考口径，不改变 formal `gold_set_300` 与 `screening_*` 的职责分离。

- 先区分 formal `gold_set/gold_set_300/` 与外部 staging `docs/gold_set_300_real_asset_staging/`；两者不是同一层，不能把 staging 进度写成 formal gold set 已完成。
- 不得只看单个 batch 文件就推断 staging 总体进度。必须对 `docs/gold_set_300_real_asset_staging/*.yaml` 做全量盘点，再判断是否已经存在“200+”已填样本。
- 盘点时不得直接按 `sample_id` 文本出现次数计数；同一 slot 的候选标识可能在多个嵌套字段重复出现，导致误判。
- staging 已填样本数必须按 slot 级“非默认特征”判断，例如：
  - `current_state != awaiting_real_input`
  - `source_record_refs` 非空
  - `candidate_prescreen_ref` 已落值
  - `canonical_url` / `candidate_document_path` 等真实来源字段已提供
- 若发现某个局部区间（例如 `121-135`）未填满，只能说明该 batch 尚未填满；不能据此推出整个 staging 目录没有两百多个样本。
- 只有在全量盘点后确认 staging 实物仍未达到当前任务明确要求时，才允许继续补样本；若实际已满足要求，不得为了“凑数”重复写入。
- 若需要补 staging 样本，只允许从现有 `docs/candidate_prescreen_workspace/` 中、且 `human_review_status = approved_for_staging` 的候选文档补入；不得从无关文档自由摘录内容伪造样本。
- 补入时必须先做非重复校验；若已有 `sample_key`，优先按 `sample_key` 去重；若是 legacy 记录缺少 `sample_key`，才回退到 `source_id + canonical_url` 语义去重。
- `docs/screening_calibration_assets/` 只允许承接并行的筛选校准资产，不得把其中数据文件写成 formal gold set、formal annotation 或 adjudication 工件。
- 若 `docs/screening_calibration_assets/` 需要新增数据文件，优先复用现有 `docs/` 外部承载文档风格；安全默认格式为 YAML 外部文档，而不是新增 formal schema、数据库契约或写入 `gold_set/`。
- `screening_positive_set`、`screening_negative_set`、`screening_boundary_set` 必须继续分目录承载；不得混成单一教材池，也不得把 `approved_for_staging / rejected_after_human_review / on_hold` 改写成正式 adjudication 语义。
- screening calibration 数据文件中只能写入当前已知、可回链的 candidate prescreen / human review / source 引用字段；不要脑补正式双标、adjudication 或新的机器可读枚举。
