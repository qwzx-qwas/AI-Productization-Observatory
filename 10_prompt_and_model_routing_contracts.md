...
2. 行内容
   ...
   - requires_schema_validation：`true`
   (6) 第 6 行
   - route_id：`route_candidate_prescreener_v1`
   - module_name：`Candidate Prescreener`
   - execution_mode：`llm_structured_json`
   - primary_engine：`llm_structured_json`
   - fallback_engine：`hold_for_human_review`
   - requires_schema_validation：`true`
...
- `Candidate Prescreener`
  - `candidate source item`: `source`, `source_window`, `external_id`, `canonical_url`, `title`, `summary`, `raw_evidence_excerpt`
  - `selection metadata`: `query_family`, `query_slice_id`, `selection_rule_version`, `time_field`
  - 输出必须保持在“候选预筛”语义层，不得伪装成正式 annotation / adjudication
