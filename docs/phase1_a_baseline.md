...
- Phase2-2 已启动状态：
  - `phase2_prompt.md`、`src/runtime/migrations.py`、`src/runtime/db_driver_readiness.py`、`src/runtime/db_driver_repository_stub.py`、`src/runtime/db_shadow.py`、`src/runtime/shadow_validation.py`、`tests/unit/test_runtime.py`、`tests/unit/test_runtime_driver_repository_stub.py`、`tests/unit/test_runtime_migrations.py`、`tests/unit/test_runtime_shadow_validation.py` 与 `tests/contract/test_contracts.py` 现一致记录：`DB runtime migration spine` 已启动，新增可替换 adapter 的 DB-side row parity + SQL claim / heartbeat / CAS reclaim contract conformance report，并补齐最小 fake-bound repository seam，用于 statement selection / bind-shape / query-shape readiness 与 fake result-row -> `TaskSnapshot` 无损映射验证；当前 row-shape conformance 已扩展到 canonical dict、mapping-like row、tuple-like row with column names、object-attribute-like row、aware datetime row、all-nullable-preserved row，以及缺字段、额外字段、rename risk、status semantic drift、timezone drift 与 nullability drift 负控；当前 conformance 已能区分 row drift、SQL contract gap、repository/query-shape gap、query/row-shape gap、semantic conformance gap 与 owner decision gap，并新增 real-driver-adapter acceptance checklist candidate standard；后续 `2026-04-28` owner-approved local-only PostgreSQL 17 disposable shadow validation 已实际连接并通过，记录 `real_db_connection = true`，但 `cutover_eligible = false`、`runtime_cutover_executed = false`，且未声明 production DB readiness 或 runtime cutover readiness。
- Phase2-3 已启动状态：
  - `phase2_prompt.md`、`16_repo_structure_and_module_mapping.md`、`src/service/operator_api.py`、`src/cli.py` 与 `tests/unit/test_operator_api.py` 现一致记录：`Service API And Operator Control Plane` 已进入 contract-first read-only increments，新增 framework-neutral operator API snapshot、operator API contract catalog、四个单视图 CLI inspect surfaces 与 plain Python `dispatch_operator_read` adapter seam，组合 mart-backed dashboard view、trace-only product drill-down、review queue view 与 task inspection view，并输出 read-only audit envelope；未传 `--mart-path` 时只从默认 fixture 派生 mart payload，不创建 runtime task；review/task inspect 只读本地 store JSON，不创建 `.lock` 文件；当前测试已覆盖 stable JSON shape、dispatch adapter、capability catalog、bad dispatch params/context、unknown dispatch params 与 invalid status / missing mart / unknown product drill-down 负控；该 read-only contract 已达到 Phase2-4 read-only frontend/serviceization 入口准备状态，但不代表 Phase2-4 frontend 已实现；该增量不冻结 dashboard framework，不新增 service write path，不执行 runtime cutover，不把 DB-backed runtime 设为默认，也不声明 production DB readiness。
- Phase2-4 / later runtime-serviceization 人工批准边界：
  ...
  - 当前 Phase2-4 已新增 framework-neutral `src/service/preview_adapter.py`、`operator-preview-model` CLI 与 optional `src/frontend/streamlit_preview.py` renderer；Streamlit 仅通过 `phase2-4-preview` optional extra 显式安装；这些入口只消费 Phase2-3 read-only operator API / mart-backed payload，保留 no-cutover guardrails，且不新增 service write path、mutation UI 或 frontend completion claim。
...
- 未决项归属与 owner 决策边界：
  - production dashboard/frontend framework、production secret backend、managed PostgreSQL vendor、object storage vendor、long-term deployment target 与 model provider vendor binding 仍保持 owner freeze pending 边界；`15_tech_stack_and_runtime.md`、`17_open_decisions_and_freeze_board.md`、`phase2_prompt.md` 与本文件一致不把这些人类保留选型写成最终依赖。本批次只提供 read-only preview/adaptor guidance、admission criteria decision packet draft、real-driver-adapter acceptance checklist candidate standard 与 provisional recommendation，不冻结产品、provider、production driver、migration runner/wrapper、real DB phase 或 cutover plan。
