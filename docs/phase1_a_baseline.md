...
- Phase2-2 已启动状态：
  - `phase2_prompt.md`、`src/runtime/migrations.py`、`src/runtime/db_driver_readiness.py`、`src/runtime/db_driver_repository_stub.py`、`src/runtime/db_shadow.py`、`tests/unit/test_runtime.py`、`tests/unit/test_runtime_driver_repository_stub.py` 与 `tests/unit/test_runtime_migrations.py` 现一致记录：`DB runtime migration spine` 已启动，新增可替换 adapter 的 DB-side row parity + SQL claim / heartbeat / CAS reclaim contract conformance report，并补齐最小 fake-bound repository seam，用于 statement selection / bind-shape / query-shape readiness 与 fake result-row -> `TaskSnapshot` 无损映射验证；当前 row-shape conformance 已扩展到 canonical dict、mapping-like row、aware datetime row、all-nullable-preserved row，以及缺字段、额外字段、rename risk、status semantic drift、timezone drift 与 nullability drift 负控；当前 conformance 已能区分 row drift、SQL contract gap、repository/query-shape gap、query/row-shape gap、semantic conformance gap 与 owner decision gap，但仍未连接真实 PostgreSQL，也未进行 runtime cutover。
- 未决项归属与 owner 决策边界：
  - `migration_tool`、`runtime_db_driver`、`managed_postgresql_vendor` 与 `secrets_manager` 仍保持 `null` / owner freeze pending 边界；`15_tech_stack_and_runtime.md`、`17_open_decisions_and_freeze_board.md`、`phase2_prompt.md` 与本文件一致不把这些未决人类选型写成最终依赖。本批次只提供 admission criteria decision packet draft 与 provisional recommendation，不冻结产品、vendor、driver 或 migration tool 名称。
