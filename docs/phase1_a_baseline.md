...
- 未决项归属与 owner 决策边界：
  - `migration_tool`、`runtime_db_driver`、`managed_postgresql_vendor` 与 `secrets_manager` 仍保持 `null` / owner freeze pending 边界；`15_tech_stack_and_runtime.md`、`17_open_decisions_and_freeze_board.md`、`phase2_prompt.md` 与本文件一致不把这些未决人类选型写成最终依赖。本批次只提供 admission criteria decision packet draft、real-driver-adapter acceptance checklist candidate standard 与 provisional recommendation，不冻结产品、vendor、driver、migration tool、real shadow DB phase 或 cutover plan。
