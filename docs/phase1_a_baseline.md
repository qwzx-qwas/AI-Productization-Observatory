...
- Phase2-3 已启动状态：
  - `phase2_prompt.md`、`16_repo_structure_and_module_mapping.md`、`src/service/operator_api.py`、`src/cli.py` 与 `tests/unit/test_operator_api.py` 现一致记录：`Service API And Operator Control Plane` 已进入首个 contract-first read-only increment，新增 framework-neutral operator API snapshot，组合 mart-backed dashboard view、trace-only product drill-down、review queue view 与 task inspection view，并输出 read-only audit envelope；未传 `--mart-path` 时只从默认 fixture 派生 mart payload，不创建 runtime task；该增量不冻结 dashboard framework，不新增 service write path，不执行 runtime cutover，不把 DB-backed runtime 设为默认，也不声明 production DB readiness。
- 未决项归属与 owner 决策边界：
