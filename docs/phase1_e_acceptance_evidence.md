...
- Phase2-2 已启动状态：
  - `phase2_prompt.md`、`src/runtime/migrations.py`、`src/runtime/db_driver_readiness.py`、`src/runtime/db_driver_repository_stub.py`、`src/runtime/db_shadow.py`、`tests/unit/test_runtime.py`、`tests/unit/test_runtime_driver_repository_stub.py`、`tests/unit/test_runtime_migrations.py` 与 `tests/contract/test_contracts.py` 现一致表述为“DB runtime migration spine 已启动，DB-shadow 可输出 DB-side row parity + SQL claim / heartbeat / CAS reclaim contract conformance report，并可显式区分 row drift、SQL contract gap、repository/query-shape gap、query/row-shape gap、semantic conformance gap 与 owner decision gap；最小 repository seam 只做 fake-bound query-shape readiness 与 fake result-row / real-driver-like row variants -> `TaskSnapshot` mapping readiness，positive variants 覆盖 mapping-like、tuple-like、attribute-like、aware datetime 与 nullable preservation，并以缺字段、额外字段、rename risk、status semantic drift、timezone drift 与 nullability drift 作为负控；real-driver-adapter acceptance checklist 仅是后续真实 shadow DB phase 的候选标准，不连接真实 PostgreSQL”，但当前仍未把 Phase1-E 的 file-backed `review_issue` / `processing_error` / `task_store` 控制平面切换到 PostgreSQL runtime backend。
- 未决项归属与 owner 决策边界：
  - `migration_tool`、`runtime_db_driver`、`managed_postgresql_vendor` 与 `secrets_manager` 仍处于保留人类选型边界并保持 `null`；本文件与 `15_tech_stack_and_runtime.md`、`phase2_prompt.md` 一致，不把 admission criteria draft、real-driver-adapter acceptance checklist candidate standard、provisional recommendation 或 fake harness validation 写成新的 Phase1-E 运行时结论。
