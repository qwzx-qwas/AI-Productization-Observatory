...
- Phase2-3 已启动状态：
  - 上述文档现一致表述为“Service API And Operator Control Plane 已启动首个 read-only / contract-first increment”，具体落地为 `src/service/operator_api.py`、`python3 -m src.cli operator-api-snapshot` 与 `tests/unit/test_operator_api.py`；当前 service snapshot 只组合 mart-backed dashboard view、trace-only product drill-down、review queue view 与 task inspection view，并输出 read-only audit envelope；未传 `--mart-path` 时只从默认 fixture 派生 mart payload，不创建 runtime task；不冻结 dashboard framework，不新增服务写路径，不执行 runtime cutover，不把 DB-backed runtime 设为默认，也不声明 production DB readiness。
- 未决项归属与 owner 决策边界：
