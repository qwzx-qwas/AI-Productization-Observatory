...
- 未决项归属与 owner 决策边界：
  - `migration_tool`、`runtime_db_driver`、`managed_postgresql_vendor`、`secrets_manager` 与 `dashboard_framework` 仍保持保留人类选型边界且在机器可读 service output 中保持 `null`；本批次文档与代码均未把这些项写成最终产品依赖。admission criteria draft、real-driver-adapter acceptance checklist 与 service API snapshot 只提供可审计接口 / 评估表面，不代表 owner decision。
