...
last_frozen_version: freeze_board_v9
---
...
2. 行内容
   ...
   (29) 第 29 行
   - decision_id：`DEC-029`
   - topic：Phase1 Product Hunt 退出门与五项审计签字流程
   - blocking：`yes`
   - owner：`project_owner`
   - affected_docs：`01`, `docs/phase1_a_baseline.md`, `docs/phase1_e_acceptance_evidence.md`, `docs/phase1_g_acceptance_evidence.md`, `14`
   - current_default：`keep Product Hunt deferred in the current phase, allow Codex to materialize audit-ready sample packs, and require human judgment plus owner sign-off before any final release go decision`
   - deadline：`TBD_HUMAN`
   - final_decision：`freeze the current Phase1 exit and audit-signoff boundary as: Product Hunt does not serve as a current Phase1 exit gate and remains deferred as fixture/replay/contract plus future live seam only; the current exit gate focuses on GitHub live evidence, dashboard/reconciliation evidence, and the five named audits = merge_spot_check / taxonomy_audit / score_audit / attention_audit / unresolved_audit; for the current gate interpretation, GitHub complete-cycle evidence is satisfied when the live GitHub 3 windows x 3 query slices matrix shows, for every window/slice combination, a recorded first run, same-window rerun, and recoverable-failure drill, while outside_window_count remains 0, durable raw is not duplicated by rerun/resume, and checkpoint/resume is verifiable from the last durable checkpoint without changing the window or advancing the final watermark early; each of those audits must follow the structured chain machine_pre_audit -> human_sampled_verdict -> owner_signoff, where Codex may materialize machine pre-audit evidence and sample packs at full current coverage, the human reviewer records sampled accept/reject judgment, and the owner records the final sign-off; machine reporting may output conditional-go or no-go tendencies but must not auto-upgrade to final go unless owner sign-off is approved and all blocking conditions are closed; final merge/release sign-off remains with the owner under the existing DEC-025 boundary`
   - status：`frozen`
   (30) 第 30 行
   - decision_id：`DEC-030`
   - topic：Phase2-4 / later runtime-serviceization implementation guidance
   - blocking：`no`
   - owner：`project_owner`
   - affected_docs：`15`, `16`, `17`, `phase2_prompt.md`, `docs/phase1_a_baseline.md`, `docs/phase1_g_acceptance_evidence.md`
   - current_default：`Phase2-4 remains read-only frontend/serviceization preview and adapter work; DB-backed runtime remains non-default and no runtime cutover is approved`
   - deadline：`TBD_HUMAN`
   - final_decision：`freeze the implementation guidance as: Streamlit is approved only for a Phase2-4 read-only preview/adaptor surface and does not freeze the production dashboard/frontend framework; psycopg3 sync is approved only as the first runtime DB driver candidate for conformance validation against existing runtime task, row-shape, timezone, CAS, and replay contracts and does not approve runtime cutover, DB-backed runtime default, or production DB readiness; raw reviewed SQL is the canonical migration source of truth, while Alembic may be evaluated later only as runner, scaffold, or history wrapper and Alembic autogenerate is not authoritative for migration semantics; managed PostgreSQL vendor selection remains deferred, Phase3 must not connect to or integrate a SQL service provider, and documentation may preserve only future-facing interfaces or evaluation hooks; environment variables are the runtime secret interface, local development may use gitignored .env files, CI may use GitHub Actions secrets, production secret backend remains deferred, and secrets, real DSNs, passwords, tokens, and real .env values must not be committed or printed`
   - status：`frozen`
...
- `2026-04-21` / `DEC-029` / owner=`project_owner` / conclusion=`confirmed with Product Hunt exit-gate deferral, GitHub full-cycle matrix interpretation, and three-stage audit workflow` / effective_scope=`Phase1 exit-gate interpretation, GitHub complete-cycle equivalence, machine pre-audit boundary, sampled human verdict, and owner sign-off baseline` / writeback_files=`01_phase_plan_and_exit_criteria.md`, `docs/phase1_a_baseline.md`, `docs/phase1_e_acceptance_evidence.md`, `docs/phase1_g_acceptance_evidence.md`, `14_test_plan_and_acceptance.md` / implementation_blocked=`no`
- `2026-04-30` / `DEC-030` / owner=`project_owner` / conclusion=`confirmed Phase2-4 read-only preview and later runtime-serviceization guidance` / effective_scope=`Streamlit preview-only allowance, psycopg3 conformance-validation candidate, raw SQL migration source of truth, deferred managed PostgreSQL vendor freeze, environment-variable runtime secret interface, no runtime cutover, no production DB readiness claim` / writeback_files=`15_tech_stack_and_runtime.md`, `16_repo_structure_and_module_mapping.md`, `phase2_prompt.md`, `docs/phase1_a_baseline.md`, `docs/phase1_g_acceptance_evidence.md` / implementation_blocked=`no`
...
- file=`15_tech_stack_and_runtime.md` / source_decisions=`DEC-007`, `DEC-027`, `DEC-030` / task4_action=`verify_or_update_note` / task4_note=`Preserve the signed local-harness caveat, PostgreSQL baseline, raw-SQL migration source of truth, environment-variable secret interface, Phase2-4 preview-only Streamlit allowance, and no-cutover boundary.`
- file=`16_repo_structure_and_module_mapping.md` / source_decisions=`DEC-007`, `DEC-030` / task4_action=`verify_or_update_note` / task4_note=`Keep repo mapping wording consistent with the signed local harness versus final DB task-table boundary and the Phase2-4 read-only preview/adaptor placement.`
...
- file=`01_phase_plan_and_exit_criteria.md` / source_decisions=`DEC-021`, `DEC-028`, `DEC-029` / task4_action=`verify_or_update_note` / task4_note=`Keep phase-gate language aligned with the signed gold-set double-annotation baseline, the MVP reference-sample-set boundary, the Product Hunt deferred exit-gate scope, and the machine_pre_audit -> human_sampled_verdict -> owner_signoff workflow.`
...
- file=`14_test_plan_and_acceptance.md` / source_decisions=`DEC-020`, `DEC-021`, `DEC-022`, `DEC-025`, `DEC-028`, `DEC-029` / task4_action=`verify_or_update_note` / task4_note=`Keep test and acceptance wording aligned with the signed taxonomy, gold-set, replay-boundary, merge-vs-release, MVP reference-sample-set baseline, Product Hunt deferred exit-gate scope, and the machine/human/owner audit workflow.`
- file=`phase2_prompt.md` / source_decisions=`DEC-030` / task4_action=`verify_or_update_note` / task4_note=`Keep Phase2-4 read-only preview/adaptor guidance aligned with Streamlit preview-only allowance, no service writes, no runtime cutover, raw SQL migration authority, deferred managed PostgreSQL vendor, and environment-variable secret interface.`
- file=`docs/phase1_a_baseline.md` / source_decisions=`DEC-029`, `DEC-030` / task4_action=`verify_or_update_note` / task4_note=`Keep the baseline narrative aligned with the signed GitHub-live/Product-Hunt-deferred exit boundary, the three-stage audit workflow, and the Phase2-4 read-only preview/adaptor boundary.`
- file=`docs/phase1_e_acceptance_evidence.md` / source_decisions=`DEC-029` / task4_action=`verify_or_update_note` / task4_note=`Keep the Phase1-E evidence wording aligned with the signed GitHub-live/Product-Hunt-deferred boundary and point final release judgment to the three-stage audit workflow in Phase1-G.`
- file=`docs/phase1_g_acceptance_evidence.md` / source_decisions=`DEC-029`, `DEC-030` / task4_action=`verify_or_update_note` / task4_note=`Keep the Phase1-G evidence package aligned with the signed machine_pre_audit -> human_sampled_verdict -> owner_signoff workflow, deferred Product Hunt exit-gate scope, and Phase2-4 read-only preview/adaptor evidence boundary.`
